...
using namespace std;
#define MAX_SIZE_BULK_READ_BUFFER 600
char bulkReadBuffer[MAX_SIZE_BULK_READ_BUFFER];
...
int main() {
	...
	for (ri tc = 0;tc < T;++tc) {
		N = fastRead_int();
		fgets(bulkReadBuffer, MAX_SIZE_BULK_READ_BUFFER, stdin);
		oddCount = 0;
		for (ri i = 0;bulkReadBuffer[i] != '\n' && i<MAX_SIZE_BULK_READ_BUFFER;++i) {
			num = 0;
		...
		if (oddCount & 1 != 0) {
			fastWriteToBuffer_int(2);
			fastWriteToBuffer_str("\n");
		}
		else {
			fastWriteToBuffer_int(1);
			fastWriteToBuffer_str("\n");
		}
	}
	flushBuffer();
